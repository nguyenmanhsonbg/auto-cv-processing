...
export interface FreelancerSelfSummary {
  freelancerId: string;
  identifier: string;
  phone: string | null;
  isActive: boolean;
...
export interface FreelancerSelfApplication {
  referralId: string;
  applicationId: string;
  candidate: {
    candidateId: string;
...
export type AmisCaptureSource = 'DOM_HEURISTIC' | 'AMIS_SAVE_RECRUITMENT_API' | 'AMIS_DETAIL_API';
...
export type AmisDiagnosticEventType =
  ...
  | 'DEBUGGER_CANDIDATE_STAGE_REQUEST_SEEN'
