...
test('keeps a failed Facebook publish tab open only while debug mode is enabled', () => {
  assert.equal(shouldKeepFacebookPublishTabOpenForDebug(true, 'FAILED'), true);
  assert.equal(shouldKeepFacebookPublishTabOpenForDebug(true, 'THREW'), true);
  assert.equal(shouldKeepFacebookPublishTabOpenForDebug(true, 'SUCCESS'), false);
  assert.equal(shouldKeepFacebookPublishTabOpenForDebug(false, 'FAILED'), false);
});

test('identifies the shared-session Facebook batch path only for multiple groups', () => {
  assert.equal(isFacebookGroupBatchTargets([
    { targetType: 'GROUP' },
    { targetType: 'GROUP' },
  ]), true);
  assert.equal(isFacebookGroupBatchTargets([{ targetType: 'GROUP' }]), false);
  assert.equal(isFacebookGroupBatchTargets([
    { targetType: 'GROUP' },
    { targetType: 'PAGE' },
  ]), false);
