...
export function shouldKeepFacebookPublishTabOpenForDebug(
  enabled: boolean,
  outcome: 'SUCCESS' | 'FAILED' | 'SKIPPED' | 'THREW',
) {
  return enabled && (outcome === 'FAILED' || outcome === 'THREW');
}

export function isFacebookGroupBatchTargets(
  targets: ReadonlyArray<{ targetType: string }>,
) {
  return targets.length > 1 && targets.every((target) => target.targetType === 'GROUP');
}
