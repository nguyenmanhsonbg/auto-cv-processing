...
ANTHROPIC_API_KEY= # Optional: AI suggestions; falls back to claude CLI
...
- `Dockerfile` — multi-stage build: `base` → `deps` → `backend-builder` / `frontend-builder` → `backend` / `frontend`
  - Backend final image installs Claude Code CLI globally (`npm install -g @anthropic-ai/claude-code`)
